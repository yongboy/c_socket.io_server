/**
**  gcc html_file.c -o html_file ./include/libev.a -lm
*/
#include <sys/types.h>
...
#define HTMLFILE_RESPONSE_MIDDLE "<script>_('8::');</script>\r\n"
...
static void format_message(const char *ori_message, char *target_message) {
    sprintf(target_message, "%X\r\n<script>_('%s');</script>\r\n", ((int)strlen(ori_message) + 23), ori_message);
}

static void write_body(client *cli, char *msg) {
    ...
    write(cli->fd, body_msg, strlen(body_msg));
}

static void write_head(client *cli, char *msg) {
    if (cli == NULL) {
        printf("the client is NULL !\n");
        return;
    }
...
static void timeout_cb(EV_P_ struct ev_timer *timer, int revents) {
    ...
    write_body(cli, HTMLFILE_RESPONSE_MIDDLE);
...
static void write_cb(struct ev_loop *loop, ev_io *w, int revents) {
    ...
    */
    ...
    if (revents & EV_WRITE) {
        ...
        write_head(cli, HTMLFILE_RESPONSE_HEADER);
        write_body(cli, HTMLFILE_RESPONSE_FIRST);
        write_body(cli, HTMLFILE_RESPONSE_WELCOME);
